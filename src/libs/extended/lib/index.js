    ...
    formulateValue: {
      handler(values) {
        ...
        if (this.isVmodeled && values && typeof values === "object") {
          ...
          keys.forEach((field) => {
            if (!utils_js.equals(values[field], this.proxy[field])) {//if (!utils_js.shallowEqualObjects(values[field], this.proxy[field])) {
              this.setFieldValue(field, values[field]);
              if (
                this.registry.has(field) &&
                !utils_js.equals(//!utils_js.shallowEqualObjects(
                  values[field],
...
const features = { formEvents, enforceNumber, textMask }; //const features = { formEvents, textMask, enforceNumber };
