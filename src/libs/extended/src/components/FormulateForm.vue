...
<script>
import { equals } from "@braid/vue-formulate/src/libs/utils.js";//import { shallowEqualObjects } from "@braid/vue-formulate/src/libs/utils.js";
    ...
    formulateValue: {
      handler(values) {
        ...
        if (this.isVmodeled && values && typeof values === "object") {
          ...
          keys.forEach((field) => {
            if (!equals(values[field], this.proxy[field])) {//if (!shallowEqualObjects(values[field], this.proxy[field])) {
